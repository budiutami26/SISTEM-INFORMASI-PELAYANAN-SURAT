                        ...
                        <table class="table">
                            ...
                            <tbody>
                                @php
                                    $no = 1;
                                @endphp
                                @foreach ($dtruangan as $item)
                                    <tr>
                                        <td>{{ $no++ }}.</td>
                                        <td>{{ $item->nama_penanggungjwb }}</td>
                                        ...
                                        @if (Auth::user()->role == 'petugas')
                                        <td>
                                            <a href="{{ route('edit_ruangan', $item->id) }}" class="btn btn-primary">Edit</a>
                                            <br>
                                            <form method="POST" action="{{ route('hapus_ruangan', $item->id) }}">
                                                @csrf
                                                @method('DELETE')
